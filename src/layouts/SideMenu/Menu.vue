...
<template>
  <SideMenuTooltip 
    ...
    "
    :class="[
      'h-[50px] flex items-center pl-5 text-black mb-0 relative rounded-full hover:text-[#fff]',
      {
...
style="
  ...
  >
   <div
    ...
    >
     <!-- <Lucide :icon="props.menu.icon" class=" " /> -->
    </div> 
    <div
    :class="[
        'xl:flex items-center  h-full ml-3 text-slate-800  ',
        { 'font-bold  hover:text-[#fff]': props.menu.active && props.level != 'first'  }, //เมนูที่ active , submenu ที่ active , เมนูต่อมาที่ไม่ใช่อันแรก
   
        { 'hover:text-[#fff]':!props.menu.active && props.level   },
   
        { 'dark:text-slate-400 hover:text-[#fff] ': !props.menu.active && props.level == 'first' }, //เมนูที่ไม่ active , submenuที่ไม่ active , เมนูแรก
    ...
    <Lucide :icon="props.menu.icon" class="ml-5 mr-4 " />
      {{ props.menu.title }}

      <div
        v-if="props.menu.subMenu"
        :class="[
          '   transition ease-in duration-100 ml-auto mr-5  xl:block',
